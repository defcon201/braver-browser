...
const os = require('os')
const { progressLog, errorLog, logUpdateStatus, logAllPatchStatus } = require('./sync/logging')
...
async function applyPatches() {
  ...
  if (hasPatchError) {
    errorLog('Exiting as not all patches were successful!')
    process.exit(1)
  }
}
...
const util = {
  ...
  updateBranding: () => {
    ...
    if (config.targetOS === 'android') {
      ...
      const androidComponentsSettingsResSource = path.join(config.projects['brave-core'].dir, 'components', 'browser_ui', 'settings', 'android', 'java', 'res')
      const androidComponentsSettingsResDest = path.join(config.srcDir, 'components', 'browser_ui', 'settings', 'android', 'java', 'res')
      const androidComponentsSiteSettingsResSource = path.join(config.projects['brave-core'].dir, 'components', 'browser_ui', 'site_settings', 'android', 'java', 'res')
      const androidComponentsSiteSettingsResDest = path.join(config.srcDir, 'components', 'browser_ui', 'site_settings', 'android', 'java', 'res')
      ...
      const copyAndroidResourceMapping = {
        ...
        [androidComponentsSettingsResSource]: [androidComponentsSettingsResDest],
        [androidComponentsSiteSettingsResSource]: [androidComponentsSiteSettingsResDest]
      }
...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  gclientSync: (reset = false, options = {}) => {
    ...
    if (prog.status === 0) {
      prog = util.run('git', ['-C', config.braveCoreDir, 'reset', '--hard', config.getProjectRef('brave-core')], git_options)
    }
